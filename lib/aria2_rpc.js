    ...
    this.onMessage = function(message){
        ...
        if(taskId){
            ...
            if(method === "aria2.onDownloadComplete"){
                _._eventEmitter.emit('downloadComplete',gid);
            }else if(method === "aria2.onDownloadStart"){
                _._eventEmitter.emit('downloadStart',gid);
            }else if(method === "aria2.onDownloadError"){
                _._eventEmitter.emit('downloadError',gid);
            }else if(method === "aria2.onDownloadPause"){
                _._eventEmitter.emit('downloadPause',gid);
            }
        }
    }
...
aria2Rpc.prototype.version = function(){
    return this.process[ariaCommand.version];
}
